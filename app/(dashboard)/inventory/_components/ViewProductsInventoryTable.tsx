...
function ProductsInventoryTable({ from, to, productId, isModal, type }: Props) {
 ...
 useEffect(() => {
    const fetchTransactions = async () => {
      const response = await fetch(`/api/transactions?productId=${productId}&type=${type}`);
  ...
  return (
    <div>
      {isModal ? (
        <TransactionTable transactions={transactions} isModal={true} />
      ) : (
